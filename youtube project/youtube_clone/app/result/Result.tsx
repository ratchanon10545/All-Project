...
const fetchMoreData = async (search_query:string) => {
    const res = await fetch(`http://localhost:3000/api/video?id=0&page=1&limit=6`);
    return res.json();
  };
...
export default function Result({data , search_query} : {data : Video[] ,search_query : string}) {
          ...
          const observer = new IntersectionObserver(
            async (entries) => {
              if (entries[0].isIntersecting && !loading) {
                ...
                fetchMoreData(search_query).then( (newData) => {
  ...
  return (
    <div>
        ...
        <div key={index} className='flex p-3 space-x-4 cursor-pointer'
        onClick={() => router.push(`/watch/${btoa(btoa(video.video_id))}`)}>
            ...
            <div className='cursor-pointer' onClick={() => router.push(`/watch/${btoa(btoa(video.video_id))}`)}>
                ...
                <p className='text-xs font-mono text-gray-500'>views {video.views}</p>
      ...
      {data.length === 0 && <p className='flex justify-center mb-5 text-xl font-bold'>No results found.</p>}
      <hr></hr>
      <div className='mt-5 text-lg font-bold'>Recommended Videos</div>
      <div>
          {items.map((video , index ) => (
             ...
             onMouseEnter={() => handleMouseEnter(index + counter)}
             onMouseLeave={() => handleMouseLeave(index + counter)}
             >
                 <video 
                 key={video.video_url}
                 
                 ref={(el) => {
                     if (el) videoRefs.current[index + counter] = el;
                   }}
                 muted
                 loop
                 onTimeUpdate={() => handleTimeUpdate(index + counter)}
                 className='rounded-lg hover:rounded-none'>
                     <source src={video.video_url} type="video/mp4" />
                     <source src={video.video_url} type="video/webm" />
                 </video>
 
                 {isHovering === index + counter &&  (
                 <div className="absolute bottom-0 left-0 w-full h-1 bg-gray-300">
                   <div
                     className="h-full bg-red-500 transition-all"
                     style={{ width: `${progress}%` }}
