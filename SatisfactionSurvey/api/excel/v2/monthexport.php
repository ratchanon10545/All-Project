...
try{
    ...
    $objPHPExcel->getProperties()->setCreator("Phichit")
        ->setLastModifiedBy("Phichit")
    ...
    $titles = array('รายงานประจำเดือน', 'ข้อเสนอแนะประจำเดือน');
    $sheet = 0;
    
    $month_thai = monththai($month);
    $year_re = $year - 543;
    foreach($titles as $value){
        if($sheet > 0){
            ...
            $sheet
                ->setCellValue('A1', 'แบบรายงานการสำรวจข้อเสนอแนะ ของผู้รับบริการ')
                ->setCellValue('A2', 'สำนักงานที่ดินจังหวัดพิจิตร')
                ->setCellValue('A3', 'ประจำเดือน '. $month_thai . ' พ.ศ. ' . $year)
                ->setCellValue('A4', 'ลำดับ')
                ->setCellValue('B4', 'วันที่')
                ->setCellValue('C4', 'ช่องบริการ')
                ->setCellValue('D4', "ข้อเสนอแนะ");
            $j = 6;
            $sql = "SELECT officer.id , officer.name, datadate.date,datadate.suggestions
            FROM officer 
            INNER JOIN ( SELECT *
                        FROM data
                        WHERE  MONTH(data.date) = ? and YEAR(data.date) = ? and suggestions != '' )  as datadate
            ON datadate.officer_id = officer.id
            ORDER BY officer.id";
            $sql = $dbh->prepare($sql);
            $sql->bindParam(1,$month);
            $sql->bindParam(2,$year_re);
            $sql->execute();
            
            $objResult = $sql->fetchAll(PDO::FETCH_ASSOC);
            foreach($objResult as $obj){
                $objPHPExcel->getActiveSheet()->setCellValue('A' . $j, $obj["id"]);
                $objPHPExcel->getActiveSheet()->setCellValue('B' . $j, convertdate($obj["date"]));
                $objPHPExcel->getActiveSheet()->setCellValue('C' . $j, $obj["name"]);
                $objPHPExcel->getActiveSheet()->setCellValue('D' . $j, $obj["suggestions"]);
                $j++;
            }
          ...
          $objPHPExcel->getActiveSheet()->getStyle('A21:B21')->getFont()->setBold(true);
        ...
        //   );
          ...
          $objPHPExcel->getActiveSheet()->getPageSetup()->setFitToHeight(0);
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                ...
                LEFT JOIN ( SELECT *
                            FROM data
                           WHERE  MONTH(data.date) = ? and YEAR(data.date) = ?)  as datadate
                ON datadate.officer_id = officer.id
                GROUP BY officer.id ORDER BY officer.id";
                $sql2 = $dbh->prepare($sql2);
                $sql2->bindParam(1,$month);
                $sql2->bindParam(2,$year_re);
                $sql2->execute();
                
                $objResult2 = $sql2->fetchAll(PDO::FETCH_ASSOC);

                $all_number_score=[0,0,0,0,0];
                $all_number_survey = 0;
                $all_avg = 0;
                ...
                foreach($objResult2 as $obj){
                    ...
                    $all_number_score[4] += $obj["point5"];
                    $all_number_score[3] += $obj["point4"];
                    $all_number_score[2] += $obj["point3"];
                    $all_number_score[1] += $obj["point2"];
                    $all_number_score[0] += $obj["point1"];
                    $all_number_survey += $obj["allpoint"];
                    $j++;
                }
                
                if($all_number_survey != 0){
                    $all_avg = round(
                        (($all_number_score[4] * 5)+($all_number_score[3] * 4)+($all_number_score[2] * 3)+($all_number_score[1] * 2)+($all_number_score[0] * 1))
                    / $all_number_survey,2);
                }
                
                $objPHPExcel->getActiveSheet()->setCellValue('C21', $all_number_score[4]);
                $objPHPExcel->getActiveSheet()->setCellValue('D21', $all_number_score[3]);
                $objPHPExcel->getActiveSheet()->setCellValue('E21', $all_number_score[2]);
                $objPHPExcel->getActiveSheet()->setCellValue('F21', $all_number_score[1]);
                $objPHPExcel->getActiveSheet()->setCellValue('G21', $all_number_score[0]);
                $objPHPExcel->getActiveSheet()->setCellValue('H21', $all_number_survey);
                $objPHPExcel->getActiveSheet()->setCellValue('I21', $all_avg);
                $objPHPExcel->getActiveSheet()->setCellValue('J21', rankpoint($all_avg));
...
function convertdate($date){
    ...
    $splitDay = explode(" ", $splitDate[2]);
    $dateEngFormat = "$splitDay[0] $month_thai $yy";;

    return  $dateEngFormat;
}
