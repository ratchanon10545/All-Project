...
const Modifyward = ({ward,places}:any) => {
    const router = useRouter()
    const [oldid,setOldid] = useState(ward.id)
    const {
        register,
        handleSubmit,
        formState: { errors },
      } = useForm<Inputs>({defaultValues:ward})
      const onSubmit: SubmitHandler<Inputs> = async (data) => {
        const listdata = {
            ...data,
            oldid
        }
        await axios.put('/api/modifyward',listdata)
        ...
        <div className='p-5'>
            <form className="max-w-md mx-auto" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="relative z-0 w-full mb-5 group">
                    ...
                    <select  {...register('placeid', { required: true })} id="underline_select" required
